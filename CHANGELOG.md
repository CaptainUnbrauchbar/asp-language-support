...
-   **Hardening**, from the findings of the security scan on this release:
    -   A glob in **Additional files** is no longer compiled to a regular expression. Patterns such as `*a*a*a*a*b.lp` took exponential time to reject a name that nearly matched, which hung the extension host for as long as it ran; matching is now linear and cannot be made to stall
    -   `#include` now stays inside the folder you have open. An include pointing outside your workspace is reported instead of read, so opening an ASP project you did not write cannot inline files from elsewhere on your disk into the program
    -   **Additional files** entries are held to the same boundary, and one that reaches out of the workspace is reported as unmatched rather than resolved
    -   The config file named by `ASPLanguage: Set Config` can no longer be pointed outside the project through a path such as `conf/../../../secrets.json`
    -   The extension declares **limited support for untrusted workspaces**: a folder you have not trusted can no longer turn on `Use PATH Clingo` by itself, since that starts a process
...
-   **The clingo command line is now visible.** Options reach a run from the settings pane, the custom arguments field and an imported config between them, and nothing said what they added up to. Every result now carries the line that actually ran, in a collapsible section beside the statistics, and it can be copied. What it reports is what clingo ended up with, so options the bundled solver refused are already gone from it, and the files an `#include` pulled into the program are named with either solver rather than hidden behind the file that was opened
...
-   **Fixed `Preprocess only` being reported as a crash with your own clingo.** A run that performs no search exits with code 0, which the extension did not count as success, so preprocessing a program ended in `Clingo process exited with code 0:` and nothing after the colon. The aspif it printed is now shown as the result it is
-   **A run from PATH that does fail now says something useful.** The message names what an exit code means where the number alone does not, reads clingo's diagnostics from stdout when that is where they were written, says which signal ended a process that was killed rather than reporting no exit code, and stops instead of trailing off after a colon when clingo explained nothing
-   Output the panel cannot take apart into answers now **fills the panel and follows it when it is resized**, instead of sitting in a box a fixed twenty lines tall with empty space underneath. It is the only thing on screen at that point, so it gets the screen
-   **Added an `Output format` setting** to the pane, controlling clingo's `--outf`, which the extension used to set to JSON without asking. All four of clingo's formats are offered and explained: `0` default text, `1` competition, `2` JSON, `3` none. **JSON stays the default because it is the only output answers can be read out of** — the answer list, the filter, `Compare answer sets`, the statistics and the copy buttons all come from it, and anything else is shown as clingo printed it. It applies to your own clingo from PATH; the bundled solver is built around `--outf=2` and is marked accordingly
